...
$stmt = $db_con->prepare("SELECT * FROM member WHERE m_id = '" . $_GET["edit"] . "' ");
$stmt->execute();
...
if (isset($_POST["update"])) {
	...
	if ($query->execute()) {
		echo "<script>alert('แก้ไขข้อมูลสำเร็จ'); window.location.href='member.php';</script>";
	} else {
		echo "<script>alert('แก้ไขข้อมูลไม่สำเร็จ'); window.location.href='member.php';</script>";
	}
}
?>
</head>
